...
def loop_glyphs(word, card, canvas_draw=None, offset_y_index=0):
    ...
    for i, char_ in enumerate(word):
        ...
        if newchar not in CHARS_ARS:
            ...
            opts_len = len(CHARS_ARS[newchar])
            opts_sel = random.randint(0, opts_len - 1)
            print("total glyph options (%s) chose (%s)" % (opts_len, opts_sel))
            for character_arr in CHARS_ARS[newchar][opts_sel]:
...
def draw_main(sentence, card, can_x=0, can_y=0):
    ...
    for i, word in enumerate(sentence.split()):
        word_b64 = base64.b64encode(word.encode('ascii')).decode('ascii')
        print("original word: %s, encoded word: %s" % (word, word_b64))
        print("drawing word (%s) %s" % (i, word_b64))
        loop_glyphs(word_b64, card, canvas_draw=canvas_draw, offset_y_index=i)
